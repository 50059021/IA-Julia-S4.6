...
void cChat::LoadChatCommands()
{
	...
	GetPrivateProfileString("Strings", "COMMAND_CLEAR", "/clearinv", COMMAND_CLEAR, sizeof(COMMAND_CLEAR), IAJuliaChatCommands);
...
*/
	...
	{
		gObj->Penalty |= 2;
		Chat.MessageLog(1, c_Blue, t_BAN, gObj, "[BanChat] Tu has sido baneado!");
...
void cChat::MessageLog(int Type, sColor LogColor, sLogType LogType, LPOBJ gObj, char *Msg, ...)
{
	...
	strcpy_s(Temp,cLocalization.Localize(Msg));
	va_list pArguments1;
...
)
{
	...
	if(lConfig == 0)
	{
		MessageLog(1, c_Red, t_COMMANDS, gObj, "[%s] Funcion desactivada.", CommandName);
	...
	{
		MessageLog(1, c_Red, t_COMMANDS, gObj, "[%s] Tu no puedes usar este comando.", CommandName);
	...
	{
		MessageLog(1, c_Red, t_COMMANDS, gObj, "[%s] Tu no tienes el level necesario, necesitas %d level mas.", CommandName, NeedLvl - gObj->Level);
	...
	{
		MessageLog(1, c_Red, t_COMMANDS, gObj, "[%s] Tu no tienes el zen requerido, Tu necesitas %d mas!", CommandName, NeedZen - gObj->Money);
	...
	{
		MessageLog(1, c_Red, t_COMMANDS, gObj, "[%s] Tu necesitas %d PcPoints mas.", CommandName, NeedPcPoint - AddTab[gObj->m_Index].PC_PlayerPoints);
	...
	{
		MessageLog(1, c_Red, t_COMMANDS, gObj, "[%s] Tu necesitas %d  WCoins mas.", CommandName, NeedWCoin - gObj->m_wCashPoint);
	...
	if (PCPoint.Config.WebEnabled && NeedWebPoints > 0)
	{
		...
		{
			MessageLog(1, c_Red, t_COMMANDS, gObj, "[%s] Tu necesitas %.2f WebPoints mas.", CommandName,NeedWebPoints - AddTab[gObj->m_Index].WEB_Points_Float);
		...
		{
			MessageLog(1, c_Red, t_COMMANDS, gObj, "[%s] Tu necesitas %d WebPoints mas.", CommandName,NeedWebPoints - AddTab[gObj->m_Index].WEB_Points);
	...
	if(CheckPlayer != 0)
	{
		if(CheckPlayer == -1)
		{
			MessageLog(1, c_Red, t_COMMANDS, gObj, "[%s] Player offline o no existe!", CommandName);
...
{
	if(CheckCommand(gObj, 1, GmSystem.cTrace, 0, 0, 0, 0, 0, 1, 0, "Trace", "/trace <Name>", Msg))
	{
		return true;
	}
   ...
   gObjTeleport(gObj->m_Index, tObj->MapNumber, (int)tObj->X, (int)tObj->Y);
	...
	{
		MessageLog(1, c_Green, t_GM, tObj, "[Trace] [Admin] %s traced", gObj->Name);
	}
	else if (GmSystem.IsAdmin(gObj->Name) == 2)
	{
		MessageLog(1, c_Green, t_GM, tObj, "[Trace] [GM] %s traced", gObj->Name);
	}
	*/
	return true;
}
...
bool cChat::MoveCommand(LPOBJ gObj,char *Msg)
{
	...
	if(!MapName[0])
	{
		MessageLog(1, c_Red, t_COMMANDS, gObj, "[Move] Usage: %s <mapa>",COMMAND_WARP);
	...
	if(Index != -1)
	{
		...
		if (!strcmp(MoveReq.MoveReqInfo[Index].MapName1,"Atlans"))
		{
			...
			{
				MessageLog(1, c_Red, t_COMMANDS, gObj, "[Move] Unable to move Atlans with that pet.");
...
bool cChat::PostCommand(LPOBJ gObj, char *Msg)
{
	...
	if(BanChat)
	{
		MessageLog(1, c_Red, t_COMMANDS, gObj, "[BanChat] Your chat is banned!");
	...
	if(Configs.Commands.PostDelay)
	{
		...
		{
			MessageLog(1, c_Red, t_COMMANDS, gObj, "[ANTI-FLOOD] Wait %d sec until you can post!",Configs.Commands.PostDelay - (Seconds - AddTab[gObj->m_Index].POST_Delay));
...
{
	if(CheckCommand(gObj,Configs.Commands.IsDrop,GmSystem.cDrop,0,0,0,0,0,3,Index,"Drop", "[Nick] /drop <ItemCount> <Type> <Id> <Lvl> <Skill> <Luck> <Opt> <Exc> <Anc>", Msg))
	{
		return true;
	}
	...
	{
		MessageLog(1,c_Red,t_GM,gObj,"[Drop] Usage: [Nick] /drop <ItemCount> <Type> <Id> <Lvl> <Skill> <Luck> <Opt> <Exc> <Anc>");
	...
	else
	{
		...
		if(tObj == gObj)
		{
			MessageLog(1, c_Green, t_GM, gObj, "[Drop] %d Item Created %d %d %d %d %d %d %d %d - Failed", ItemArray[0],ItemArray[1],ItemArray[2], ItemArray[3], ItemArray[4], ItemArray[5], ItemArray[6], ItemArray[7], ItemArray[8]);
		}
		else
		{
			Message(1, gObj, "[Drop] You recived %d from %s!", gObj->Name, ItemArray[0]);
			MessageLog(1, c_Green, t_GM, gObj, "[Drop] %d Item Created to %s %d %d %d %d %d %d %d %d - Success", ItemArray[0],tObj->Name,ItemArray[1],ItemArray[2], ItemArray[3], ItemArray[4], ItemArray[5], ItemArray[6], ItemArray[7], ItemArray[8]);
		}
	}
...
bool cChat::AddCommands(LPOBJ gObj, char *Msg, int Type)
{
	...
	if(Configs.Commands.AddPointEffect > 0)
	{
		...
		Message(1,gObj, "[AddStats] Your stats successfully added!");
	}
...
bool cChat::CheckVIPCommand(LPOBJ gObj, char *Msg)
{
	...
	{
		MessageLog(1, c_Red, t_VIP, gObj, "[VIP] You have %d min(s) left.", AddTab[gObj->m_Index].VIP_Min,Vip.Config.VIPState[AddTab[gObj->m_Index].VIP_Type].VIPName);
	}
	else
	{
		MessageLog(1, c_Red, t_VIP, gObj, "[VIP] You haven't bought VIP yet. Say %s and see the VIP Plans!", COMMAND_VIPLIST);
	}
...
bool cChat::VIPListCommand(LPOBJ gObj, char *Msg)
{
	...
	for(int i = 1; i <= Vip.Config.NumStates; i++)
	{
		...
		(
			...
			t_VIP,
			gObj,
			"[VipList] %s - %d PCPoints, %d WCoins, %d Zen, %d minimum hours, %d - maximum hours (EXP %d - DROP %d - Zen %d)",
			Vip.Config.VIPState[i].VIPName,
			Vip.Config.VIPState[i].CostPCPoints,
			Vip.Config.VIPState[i].CostWCoins,
			Vip.Config.VIPState[i].CostZen,
			Vip.Config.VIPState[i].MinHours,
			Vip.Config.VIPState[i].MaxHours,
			Vip.Config.VIPState[i].BonusExp,
			Vip.Config.VIPState[i].BonusDrop,
			Vip.Config.VIPState[i].BonusZen
		);
	}
...
{
	if (CheckCommand(gObj, Vip.Config.Enabled, GmSystem.NONE, 0, 0, 0, 0, 0, 1, 0, "VIPBuy", "/vipbuy <name> <hours>", Msg))
	{
		return true;
	}
	...
	(
		CheckCommand(gObj,
		Vip.Config.VIPState[RealState].EnabledCmd,
		GmSystem.NONE,
		Vip.Config.VIPState[RealState].CostZen * Hours,
		Vip.Config.VIPState[RealState].CostPCPoints * Hours,
		Vip.Config.VIPState[RealState].CostWCoins * Hours,
		Vip.Config.VIPState[RealState].CostWebPoints * Hours,
		0,
		1,
		0,
		"VIPBuy", "/vipbuy <name> <hours>", Msg
		)
	)
	{
		return true;
	...
	TakeCommand
	(
		gObj,Vip.Config.VIPState[RealState].CostZen * Hours,
		Vip.Config.VIPState[RealState].CostPCPoints * Hours,
		Vip.Config.VIPState[RealState].CostWCoins * Hours,
		Vip.Config.VIPState[RealState].CostWebPoints * Hours,
		"BuyVIP"
	...
	(
		"UPDATE MEMB_INFO SET %s = (%s + %d), %s = %d WHERE memb___id = '%s'",
		Vip.Config.ColumnDate,
		Vip.Config.ColumnDate,
		Hours * 60,
		Vip.Config.Column,
		RealState,
...
bool cChat::VipOnCommand(LPOBJ gObj)
{
	...
	Me_MuOnlineQuery.ExecQuery("UPDATE MEMB_INFO SET VIP_ONOFF = 1 WHERE memb___id = '%s'", gObj->AccountID);
	Me_MuOnlineQuery.Fetch();
	Me_MuOnlineQuery.Close();
...
bool cChat::VipOffCommand(LPOBJ gObj)
{
	...
	Me_MuOnlineQuery.ExecQuery("UPDATE MEMB_INFO SET VIP_ONOFF = 0 WHERE memb___id = '%s'", gObj->AccountID);
	Me_MuOnlineQuery.Fetch();
	Me_MuOnlineQuery.Close();
...
bool cChat::EvoCommand(LPOBJ gObj)
{
	...
	{	
		...
		Chat.MessageLog(1, c_Blue, t_COMMANDS, gObj,"[Evo] Congratulations! Now you is a Master.");
	}
	else if((gObj->DbClass == 3) || (gObj->DbClass == 19) || (gObj->DbClass == 35) || (gObj->DbClass == 50) || (gObj->DbClass == 66) || (gObj->DbClass == 83))
	{
		Chat.MessageLog(1, c_Red, t_COMMANDS, gObj,"[Evo] Sorry, you is already a master.");
	}
	else
	{
		Chat.MessageLog(1, c_Red, t_COMMANDS, gObj,"[Evo] You need to have the second class before.");
	}
...
bool cChat::ZenCommand(LPOBJ gObj,char *Msg)
{
	...
	else
	{
		...
		if(Buffer > 0)
		{
			...
			{
				MessageLog(1, c_Red, t_COMMANDS, gObj, "[Zen] Zen can't be less than 1 and more than 2000000000!");
			}
			else
			{
				gObj->Money += Buffer;
...
bool cChat::ClearInvCommand(LPOBJ gObj)
{
	...
	else
	{
		...
		*/
		...
		Chat.MessageLog(1, c_Blue, t_COMMANDS, gObj,"[Clear Inventory] You have cleaned your inventory!");
	}
...
bool cChat::NoCommand(LPOBJ gObj)
{
	...
	switch(TypeMarry)
	{
		case 0:
		{
			if(nObj == NULL)
			{
				Chat.Message(1,gObj,"You called off the wedding");
			}
			else
			{
				// Private
				Monster.NPCMessage(gObj->m_Index,nObj, "You called off the wedding");
			}
		...
		default:
		{
			...
			Monster.NPCMessageNear(nObj, "The wedding was canceled by %s!", gObj->Name);

			Chat.MessageAllLog(0, 0, c_Green, t_Default, gObj, "[Marriage] The wedding was canceled by %s!", gObj->Name);
