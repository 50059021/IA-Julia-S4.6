...
{		  			   
	...
	Config.DuelGate					= Configs.GetInt(0, 65535,				294,	"DuelManager",  "DuelOutGate",					IAJuliaCommon);
}
...
{
	if(this->g_DuelRooms[iRoom].szlpObjOne != NULL) //  ????? ????? 1
	{
	...
	// ----
	if(this->g_DuelRooms[iRoom].szlpObjTy != NULL)  // ????? ????? 2
	{
